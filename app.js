...
document.getElementById('start-player1').addEventListener('click', function() {
    ...
    // Notify VideoStreamIntegrated of starting player selection (activates button controls)
    if (window.VideoStreamIntegrated && window.VideoStreamIntegrated.onStartingPlayerSelected) {
        window.VideoStreamIntegrated.onStartingPlayerSelected(1);
    }
...
document.getElementById('start-player2').addEventListener('click', function() {
    ...
    // Notify VideoStreamIntegrated of starting player selection (activates button controls)
    if (window.VideoStreamIntegrated && window.VideoStreamIntegrated.onStartingPlayerSelected) {
        window.VideoStreamIntegrated.onStartingPlayerSelected(2);
    }
...
function submitTurn() {
    ...
    // Notify VideoStreamIntegrated that turn was submitted (switches button controls to next player)
    if (window.VideoStreamIntegrated && window.VideoStreamIntegrated.onTurnSubmitted) {
        const turnData = {
            player: gameState.currentPlayer === 1 ? 2 : 1, // Was just switched, so use old player
            score: gameState.turnTotal,
            playerScores: {
        ...
        window.VideoStreamIntegrated.onTurnSubmitted(turnData);
    }
}
...
function initializeEventHandlers() {
    ...
    numButtons.forEach(btn => {
        btn.addEventListener('click', function() {
            // Check if current player has control (for turn-based multi-player)
            if (window.VideoStreamIntegrated && window.VideoStreamIntegrated.state) {
                if (!window.VideoStreamIntegrated.state.buttonState.enabled) {
                    console.log('🔒 Button locked - not your turn');
                    return; // Ignore click if buttons are locked
                }
            }
    ...
    document.getElementById('submit-btn')?.addEventListener('click', function() {
        ...
        if (window.VideoStreamIntegrated && window.VideoStreamIntegrated.state) {
            if (!window.VideoStreamIntegrated.state.buttonState.enabled) {
                console.log('🔒 Submit button locked - not your turn');
                return; // Ignore click if buttons are locked
            }
        }
    ...
    document.addEventListener('keydown', function(event) {
        ...
        if (window.VideoStreamIntegrated && window.VideoStreamIntegrated.state) {
            if (!window.VideoStreamIntegrated.state.buttonState.enabled) {
                console.log('⌨️ Keyboard locked - not your turn');
                return; // Ignore keyboard input if buttons are locked
            }
        }
